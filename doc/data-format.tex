...
\begin{itemize}
    ...
    \item independent of a particular desktop environment, Linux
    distribution, etc.
...
\begin{itemize}
    ...
    \item flush the zlib compressor, base64-encode the tail and write it to the
    report file, again with a space prefix.
\end{itemize}
...
problem type and give information about the date, Linux distribution and user
environment.
...
\begin{description}
    ...
    (for failed install/upgrade of a distribution package), and \verb!Bug! (for
    general bug reports)
    ...
    \item [DistroRelease:] (optional) Output of \verb!lsb_release -sir!

    \item [Architecture:] (optional) Distribution specific notation of
...
\begin{description}
    \item [CoreDump:] (optional) core dump (binary value)

    \item [Stacktrace:] (optional) Stack trace as produced by gdb's \verb!bt full!
    command

    \item [ThreadStacktrace:] (optional) Threaded stack trace as produced by
    gdb's \verb!thread apply all bt full! command
    ...
    \item [Registers:] (optional) Register dump as produced by gdb's 
    \verb!info registers! command

    \item [Disassembly:] (optional) Disassembly of the code leading to the
    crash as produced by gdb's \verb!x/16i $pc! command
\end{description}
